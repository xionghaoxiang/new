...
void data_transition(void)
{
	...
	//差速控制解算Diff_Car: 
	Send_Data.Sensor_Str.X_speed = ((MotorA.Current_Encoder+MotorB.Current_Encoder)/2)*100; 
	Send_Data.Sensor_Str.Y_speed = 0;
	Send_Data.Sensor_Str.Z_speed = Yaw;

	
	//The acceleration of the triaxial acceleration //加速度计三轴加速度
	Send_Data.Sensor_Str.Accelerometer.X_data= total_distance;  //加速度计Y轴转换到ROS坐标X轴
	Send_Data.Sensor_Str.Accelerometer.Y_data=RunTimer_GetMs()/100;  //加速度计X轴转换到ROS坐标Y轴
	Send_Data.Sensor_Str.Accelerometer.Z_data= Accel_Z;  //加速度计Z轴转换到ROS坐标Z轴
	
	//The Angle velocity of the triaxial velocity //角速度计三轴角速度
	Send_Data.Sensor_Str.Gyroscope.X_data= Gyro_Y;  //角速度计Y轴转换到ROS坐标X轴
	Send_Data.Sensor_Str.Gyroscope.Y_data=-Gyro_X;  //角速度计X轴转换到ROS坐标Y轴
	if(Flag_Stop==0) 
...
//						Move_Y=XYZ_Target_Speed_transition(Receive_Data.buffer[5],Receive_Data.buffer[6]);
							...
							Move_Z = target_limit_float(Move_Z,-Pi/3,Pi/3);   //限幅
...
{	
	...
	if(USART_GetITStatus(ROS_USARTx, USART_IT_RXNE) != RESET)
    {
	u8	RxData = USART_ReceiveData(UART4);//Read the data //读取数据
//	OLED_ShowString(50,00,"X:");
...
//		{
			...
			OLED_ShowNumber(00,00,RxData,3,12);
//			pRxPacket ++;				
//			if (pRxPacket >= 1)			
//			{			
//				RxState = 2;			
...
//		}
        ...
        USART_ClearITPendingBit(ROS_USARTx, USART_IT_RXNE);
    }
}
...
{	
	...
	if(USART_GetITStatus(UNUSED_USARTx, USART_IT_RXNE) != RESET)
    {
	u8	RxData = USART_ReceiveData(USART2);//Read the data //读取数据

			Serial_RxPacket1[pRxPacket] = RxData;	
			Serial_RxFlag1=RxData;
        interrupt_count++;
        if(RxData!=0 && RxStateflag==0)
		{
			Serial_RxFlag2=RxData;
			RxStateflag=1;
		}
        // 如果中断过于频繁，可能是没有正确处理
        ...
        USART_ClearITPendingBit(UNUSED_USARTx, USART_IT_RXNE);
    }
	    // 一定要清除所有可能的中断标志
