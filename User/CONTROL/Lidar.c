...
PointDataProcessDef Dataprocess[420];      //用于小车避障、跟随、走直线、ELE雷达避障的雷达数据
...
void data_process(void)
{
	...
	if(test_once_flag) //这帧数据要做数据切割的情况（这帧数据包含了这一圈的数据和下一圈的数据）
	{
		 ...
		 memcpy(Dataprocess,PointDataProcess,sizeof(PointDataProcessDef)*420);//这时一整圈的数据已经收集完成，把一整圈数据复制到另一个数组进行使用
 ...
 {
	  	for(k=0;k<12;k++)    //把一帧数据存放在distance_sum数组里
	    {
